...
fastify.get('/user/:id', function (req, reply) {
    fastify.mysql.query('SELECT id, name, age FROM user WHERE id=?', [req.params.id], function onResult(err, result) {
...
fastify.post('/user', (req, reply) => {
    ...
    fastify.mysql.query('INSERT INTO user (name, age) VALUES (?, ?)', [name, age], (err, result) => {
        if (err) {
...
fastify.patch('/user/:id', (req, reply) => {
    ...
    fastify.mysql.query('UPDATE user SET name = ?, age = ? WHERE id = ?', [name, age, id], (err, result) => {
        if (err) {
            reply.code(500).send(err);
        }
...
fastify.delete('/user/:id', (req, reply) => {
    const { id } = req.params;
    fastify.mysql.query('DELETE from user WHERE id = ?', [id], (err, result) => {
        if (err) {
            reply.code(500).send(err);
        }
        else {
